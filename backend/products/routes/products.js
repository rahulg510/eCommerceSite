...
router.get("/single-product", (req, res) => {
	...
	let p = products.find(i=>i._id === id);
	let product = {
		_id: id,
		name: "wooden desk",
		price: 40099,
...
router.delete("/", async (req,res) => {
	...
	try{
		let product = await Product.findOneAndDelete({__id:id, owner: user}).lean();
		if(product){
			res.json(product);
		}
		else{
